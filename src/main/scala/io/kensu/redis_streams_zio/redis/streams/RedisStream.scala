...
object RedisStream {
  ...
  def live[S <: StreamInstance: Tag]: ZLayer[Has[S] with RedisClient, Nothing, RedisStream[S]] =
    ZLayer.fromFunction(env =>
      new Service[S] {
        import scala.jdk.CollectionConverters._

        private val redisson = env.get[RedissonClient]
        private val instance = env.get[S]
