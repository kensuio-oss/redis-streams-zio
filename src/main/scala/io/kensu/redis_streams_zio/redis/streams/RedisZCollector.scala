...
object RedisZCollector {
  ...
  ): ZIO[RedisStream[S] with Has[C] with Logging with Clock, Throwable, Unit] =
    ZIO.service[C].flatMap { config =>
      getPendingEvents
        .repeat(repeatStrategy.getOrElse(Schedule.fixed(Duration.fromScala(config.claiming.repeatEvery))))
        .delay(Duration.fromScala(config.claiming.initialDelay))
    ...
    ZIO.service[C].zip(ZIO.service[RedisStream.Service[S]]).flatMap {
      case (config, redisStream) =>
        ...
        log.debug(s"Listing pending messages for group $group and consumer $consumer") *>
          redisStream.listPending(group, 100).flatMap { pendingEntries =>
        ...
        NonEmptyChunk.fromChunk(messageIds) match {
          ...
          case Some(ids) =>
            log.debug(s"Attempt to claim batch of $batchSize messages $commonLogMsg $ids") *>
              redisStream
                .fastClaim(group, consumer, Duration.fromScala(config.claiming.maxIdleTime), ids)
                .map(_.size.toLong)
                .tapBoth(
